...
7. **CDN 校验(生成后必做)**:对主源 URL 各执行一次可达性校验(curl 或等效方式);不可达则按降级链换源并更新全部文件;全部源不可达(纯内网)时改用内联最小样式方案并在 index.html 顶部注释声明环境限制。
8. **登记**:`memory/outputs-index.md` 不存在时按 `memory/README.md` 规范创建;登记原型产出与关键词(UI、原型、Tailwind)。
9. **自检**:逐项执行下方自检清单后回传。
...
- [ ] App 端:393×852 画布 + iOS 状态栏 + 底部 TabBar;圆角外框仅在 index 设备壳

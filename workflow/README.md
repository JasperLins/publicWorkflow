...
7. **并行问题合并协议**:并行子代理的批量问题**不直接写共享的 questions-stageN.md**,只在回传中提交;由主代理统一合并编号落盘(避免互相覆盖)。同一子代理线程内串行执行多个 skill 时,可由其直接落盘 questions(无并行覆盖风险)。
...
- **回填循环(必做)**:技术栈定稿后,由**高级全栈工程师**按选型回填校准 `feature-list.md` 工时(变更写入该文档「版本/变更记录」),重大偏差(>±20%)列入下一轮批量提问。假设账本中依赖**运行时资源**的校验项(如准确率基准测试),可将校验时点改挂开发期首周,并在假设账本标注新时点(须经用户确认)。
- **上游参数回填**:回填中需把已定决策参数(如 N/M 额度)写回上游文档(PRD 验收标准等)时——上游文档升小版本、状态改「草稿(待本阶段批量提问确认)」,并逐处注明依据决策号;随本阶段门禁确认后统一回写「已确认」。
- 完成标准:推荐方案含打分对比与架构图;第三方成本含免费额度/预估月费两档(可按用户规模分档);功能清单已回填校准,**且回填结果经 doc-quality-reviewer 快检(主代理执行即可:变更记录完整性 + 算术可复算 + 偏差口径)**。
...
- 完成标准:按 **AI 竞争档**定价(委托方锚点优先,人日仅参考);现金红线核对;利润三档如实;报价单可直接发甲方(无内部数字泄露);**质量红线入单(低价不减质,范围/验收标准不缩水)**。
